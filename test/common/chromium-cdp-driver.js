...
function setupLogging(client) {
  ...
  client.Runtime.exceptionThrown((info) => {
    ...
    if (
      details.exception &&
      details.exception.className === "PhWaitCondTimeout"
    )
      return;
  ...
  client.Log.entryAdded((entry) => {
    ...
    if ((msg.text || "").indexOf("Refused to apply inline style") >= 0) {
      ...
      if (
        msg.stackTrace &&
        msg.stackTrace.callFrames &&
        msg.stackTrace.callFrames[0].functionName === "makeStyleTag"
      )
        return;
      ...
      if (
        msg.stackTrace &&
        msg.stackTrace.callFrames &&
        msg.stackTrace.callFrames[0].functionName.length == 1
      )
...
function setupSSLCertHandling(client) {
  ...
  client.Security.certificateError((info) => {
    process.stderr.write(
      `CDP: Security.certificateError ${JSON.stringify(
        info
      )}; action: ${ssl_bad_certificate_action}\n`
    );
